jQuery(document).ready(function($) {
    ...
    $( document ).on( 'click', '.widget_multi_add_field', kiddiz_widget_multi_add_field )
        .on( 'change', '.widget_multi_single_field', kiddiz_widget_multi_single_field )
        .on( 'click', '.widget_multi_remove_field', kiddiz_widget_multi_remove_field )

    function kiddiz_set_multi_add_field() {
    ...
    kiddiz_set_multi_add_field();
    
    function kiddiz_widget_multi_add_field(e) {
        ...
        e.preventDefault();
            ...
            kiddiz_widget_multi_write( $control );
    }

    function kiddiz_widget_multi_single_field() {
        var $control = $( this ).parents( '.widget_multi_input' );
        kiddiz_widget_multi_write( $control );
    }

    function kiddiz_widget_multi_remove_field(e) {
        e.preventDefault();
        var $this = $( this );
        var $control = $this.parents( '.widget_multi_input' );
        $this.parent().remove();
    ...
    function kiddiz_widget_multi_write( $element) {
    ...
    $(document).on("click", ".kiddiz-accordion", function (e) {
        e.preventDefault();
        var $button = $(this);

        $button.next('.kiddiz-panel').slideToggle();
    ...
    function kiddiz_contentTypeChange() {
    ...
    kiddiz_contentTypeChange();

    function kiddiz_widget_chosen() {
        $(".kiddiz-widget-chosen-select").chosen({
            width: "100%"
        });
    }
    kiddiz_widget_chosen();

    $(document).on('widget-updated widget-added', function(){

        // icon picker
        $('.kiddiz-icon-picker').each( function() {
