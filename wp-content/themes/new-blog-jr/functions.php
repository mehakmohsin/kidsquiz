...
function new_blog_jr_customize_other( $wp_customize ) {
	...
	$wp_customize->add_setting( 'new_blog_jr_feature_post_noofpost', 
	array(
		'default'     => 3,
		'sanitize_callback' => 'new_blog_sanitize_positive_integer'
	) );

	$wp_customize->add_control( 'new_blog_jr_feature_post_noofpost', 
		array(
			'label' 	=> __( 'Number of post to show', 'new-blog-jr' ),
			'section'	=> 'new_blog_feature_post_section',
			'settings' 	=> 'new_blog_jr_feature_post_noofpost',
			'type' 		=> 'number',
		) 
	);
}
