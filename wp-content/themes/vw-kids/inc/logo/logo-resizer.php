 ...
 */
function vw_kids_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size','vw-kids' ),
	...
	$wp_customize->add_setting( 'vw_kids_logo_title_hide_show',array(
      'default' => 1,
      'transport' => 'refresh',
      'sanitize_callback' => 'vw_kids_switch_sanitization'
    ));  
    $wp_customize->add_control( new VW_Kids_Toggle_Switch_Custom_Control( $wp_customize, 'vw_kids_logo_title_hide_show',array(
      'label' => esc_html__( 'Show / Hide Site Title','vw-kids' ),
      'priority'    => 10,
      'section' => 'title_tagline'
    )));

    $wp_customize->add_setting( 'vw_kids_tagline_hide_show',array(
      'default' => 1,
      'transport' => 'refresh',
      'sanitize_callback' => 'vw_kids_switch_sanitization'
    ));  
    $wp_customize->add_control( new VW_Kids_Toggle_Switch_Custom_Control( $wp_customize, 'vw_kids_tagline_hide_show',array(
      'label' => esc_html__( 'Show / Hide Site Tagline','vw-kids' ),
      'priority'    => 11,
      'section' => 'title_tagline'
    )));
}
add_action( 'customize_register', 'vw_kids_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function vw_kids_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = vw_kids_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = vw_kids_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'vw_kids_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function vw_kids_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
